...
class _AddMedicationScreenState extends ConsumerState<AddMedicationScreen> {
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            Text('أوقات الجرعات', style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 6),
            for (var i = 0; i < _times.length; i++)
              ListTile(
                contentPadding: EdgeInsets.zero,
                leading: const Icon(Icons.schedule),
                title: Text('الجرعة ${i + 1}'),
  ...
  Future<void> _save() async {
    ...
    final allDoses = await ref.read(medicationRepositoryProvider).getAllDoseEvents();
    await ref.read(notificationServiceProvider).scheduleUpcoming(
          allDoses.where((item) => item.event.scheduledAt.isAfter(DateTime.now())).toList(),
          settings,
        );
    refreshApp(ref);
